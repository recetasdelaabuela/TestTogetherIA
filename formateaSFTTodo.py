...
def formateaSFT(logger, nombre_archivo, nombre_archivo_nuevo):
    try:
        ...
        + str(row['Pasos']) + ' Ingredientes ' + str(row['Ingredientes']) + ' Pais ' + str(row['Pais']) + ' Duracion ' \
        + str(row['Duracion']) + ' Categoria ' + str(row['Categoria']) + ' Tiempo ' + str(row['Tiempo']) + ' Dificultad ' + str(row['Dificultad']) , axis=1)
